...
const chapter = [start, ...main, end]
let index = 0

const goToChapter = (direction) => {
  const way = direction === "up" ? 1 : -1;
  const newIndex = Math.max(0, Math.min(chapter.length - 1, index + way));

  if (newIndex !== index) {
    index = newIndex;
    chapter[index].scrollIntoView({ behavior: 'smooth', block: 'start'});
  }}

upBtn.addEventListener("click", () => goToChapter("up"));
downBtn.addEventListener("click", () => goToChapter("down"));
downloadBtn.addEventListener("click", () => {
  chapter[3].scrollIntoView({ behavior: 'smooth', block: 'start'});
  index = 3;
});
